...
#### turning exudation and priming on
################################################################################
...
Perform_Analytical_Run10 <- function(f.flag = 1, cDF, eDF) {
    ...
    CslowLong_no_priming <- omegas*VLongN$equilNPP/pass$decomp_s/(1-pass$qsq)*1000.0
...
#                                  NinL = Nin)
#    
#    # calculate N required to close stoichiometric gap of active SOM
#    rhizo_cue <- 0.3
#    c_into_exud <- npp * a$ar * a$ariz * rhizo_cue
#    n_into_exud <- c_into_exud * a$nr
#    
#    # Calculate medium term nutrient constraint
