...
### Burial fractions from passive pool
### considering exudation and priming effect
slow_priming <- function(df, a, npp) {
    
    len <- length(df)
    
    decomp_p <- 0.00013*52*Actsoil(Tsoil)   # decomposition of passive pool per year without priming
    decomp_s_old <- 0.0038*52*Actsoil(Tsoil)    # decomposition of slow pool per year without priming
    ...
    c_into_active <- npp * a$ar * a$ariz * rhizo_cue * 1000.0
    ...
    #decomp_s <- (n_active_gap / (CslowLong_no_priming * ncs)) + decomp_s_old
    decomp_s <- decomp_s_old * (1 + km) * pmax(c_into_active/(c_into_active + km), 0.3)
