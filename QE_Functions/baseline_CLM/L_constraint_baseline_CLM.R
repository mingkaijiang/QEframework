...
L_constraint_baseline_CLM <- function(df, a, C_pass, Nin_L) {
    ...
    # in g C m-2 yr-1
    #NPP_NC <- U0 / (leachn + 1) / (nburial + (a$nfl*a$af + a$nr*a$ar + a$nw*a$aw))
    #NPP_NC <- U0 / (leachn + 1) / nburial
    NPP_NC <- U0 / (leachn + 1) / ((1+omega_ap) * (a$nfl*a$af + a$nr*a$ar + a$nw*a$aw) - nburial)
    ...
    out <- data.frame(NPP, nburial)
    
    colnames(out) <- c("NPP", "nburial")
