...
#### Analytical script Run 15
####
#### Assumptions:
#### 1. CLM model C cost of N uptake
#### 2. fixed wood NC
#### 3. N uptake satisfied by N active only
####
################################################################################
#### Functions
Perform_Analytical_Run15 <- function(f.flag = 1) {
    #### Function to perform analytical run 15 simulations
    ...
    VL <- VL_constraint_FUN_3(a=a_nf)
    
    ### calculate VL equil potential NPP
    VL_eq <- solve_VL_FUN_3(CO2=CO2_1)
    ...
    L <- L_constraint_FUN_3(df=nfseq, a=a_nf, 
                          C_pass=C_pass_VL,
                          Nin_L = Nin)
    
    ### Find long term equilibrium point
    L_eq <- solve_L_FUN_3(CO2=CO2_1, C_pass=C_pass_VL, Nin_L = Nin)
    ...
    M <- M_constraint_FUN_3(df=nfseq,a=a_nf, 
                          C_pass=C_pass_VL, 
                          C_slow=C_slow_L, 
                          Nin_L = Nin+N_wood_L)
    
    ### calculate M equilibrium point
    M_eq <- solve_M_FUN_3(CO2=CO2_1, 
                        C_pass=C_pass_VL, 
                        C_slow=C_slow_L, 
                        Nin_L = Nin+N_wood_L)
    ...
    VL_eq <- solve_VL_FUN_3(CO2=CO2_2)
    
    ### calculate nw and nr for VL equilibrated nf value
    a_eq <- alloc(VL_eq$nf)
    
    ### Find long term equilibrium point
    L_eq <- solve_L_FUN_3(CO2=CO2_2, C_pass=C_pass_VL, Nin_L = Nin)
    
    ### L pot allocation
    a_eq <- alloc(L_eq$nf)
    
    ### Find medium term equilibrium point
    M_eq <- solve_M_FUN_3(CO2=CO2_2, 
                        C_pass=C_pass_VL, 
                        C_slow=C_slow_L, 
                        Nin_L = Nin+N_wood_L)
    ...
    if (f.flag == 1) {
        ...
        tiff("Plots/Analytical_Run15_2d.tiff",
