...
#### Analytical script Run 51
####
#### Assumptions:
#### 1. baseline N cycle
#### 2. Variable wood NC
#### 3. GDAY uptake as a function of root and mineral N
####    that is, saturaing function of root biomass
#### 4. ar = 0.4
#### 
####
################################################################################
#### Functions
Perform_Analytical_Run51 <- function(f.flag = 1) {
    #### Function to perform analytical run 51 simulations
    ...
    ######### Main program
    source("Parameters/Analytical_Run51_Parameters.R")
    ...
    L <- L_constraint_root_gday(df=nfseq, a=a_nf, 
                                C_pass=C_pass_VL,
                                Nin_L = Nin)
    
    ### Find long term equilibrium point
    L_eq <- solve_L_full_root_gday(CO2=CO2_1, C_pass=C_pass_VL, Nin_L = Nin)
    ...
    M <- M_constraint_root_gday(df=nfseq,a=a_nf, 
                                C_pass=C_pass_VL, 
                                C_slow=C_slow_L, 
                                Nin_L = Nin+N_wood_L)
    
    ### calculate M equilibrium point
    M_eq <- solve_M_full_root_gday(CO2=CO2_1, 
                                   C_pass=C_pass_VL, 
                                   C_slow=C_slow_L, 
                                   Nin_L = Nin+N_wood_L)
    ...
    VL_eq <- solve_VL_full_root_gday(CO2=CO2_2)
    
    ### Find long term equilibrium point
    L_eq <- solve_L_full_root_gday(CO2=CO2_2, C_pass=C_pass_VL, Nin_L = Nin)
    
    ### Find medium term equilibrium point
    M_eq <- solve_M_full_root_gday(CO2=CO2_2, 
                                   C_pass=C_pass_VL, 
                                   C_slow=C_slow_L, 
                                   Nin_L = Nin+N_wood_L)
    ...
    if (f.flag == 1) {
        ...
        tiff("Plots/Analytical_Run51_2d.tiff",
