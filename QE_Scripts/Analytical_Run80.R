...
#### Analytical script Run 80
####
#### Assumptions:
#### 1. turn root exudation on
#### 2. turn priming effect off (effect on slow SOM pool)
#### 3. N only model with explicit mineral N considered
#### 4. Variable wood stoichiometry 
####
################################################################################
#### Functions
Perform_Analytical_Run80 <- function(f.flag = 1) {
    #### Function to perform analytical run 80 simulations
    ...
    ######### Main program
    source("Parameters/Analytical_Run8_Parameters.R")
    ...
    M <- M_constraint_no_prim(df=nfseq,a=a_nf, 
                           C_pass=C_pass_VL, 
                           C_slow=C_slow_L_prim_off, 
                           Nin_L = Nin+N_wood_L)
    
    ### calculate M equilibrium point
    M_eq <- solve_M_no_prim(CO2=CO2_1, 
                              C_pass=C_pass_VL, 
                              C_slow=C_slow_L_prim_off, 
                              Nin_L = Nin+N_wood_L)
    ...
    M_eq <- solve_M_no_prim(CO2=CO2_2, 
                         C_pass=C_pass_VL, 
                         C_slow=C_slow_L_prim_off, 
                         Nin_L = Nin+N_wood_L)
    ...
    if (f.flag == 1) {
        ...
        tiff("Plots/Analytical_Run80_2d.tiff",
